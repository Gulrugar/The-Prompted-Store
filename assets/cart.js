...
class CartItems extends HTMLElement {
  constructor() {
    ...
    if (cartUpsellToggle) {
      cartUpsellToggle.addEventListener(
        "change",
        this.onCartUpsellToggle.bind(this)
      );
    }
  }
    ...
    if (this.tagName === "CART-DRAWER-ITEMS") {
      fetch(`${routes.cart_url}?section_id=cart-drawer`)
        ...
        .then((responseText) => {
          ...
          const parsedStateElement = html.querySelector(
            "[data-cart-drawer-state]"
          );
          const parsedState = parsedStateElement
            ? JSON.parse(parsedStateElement.textContent)
            : null;
          this.updateCartUpsellToggleState();
          if (parsedState) {
            this.updateCartUpsellVisibility(parsedState.item_count);
  ...
  updateCartUpsellToggleState() {
    ...
    const upsellItem = Array.from(cartItems).find((item) => {
      ...
      return (
        input &&
        input.getAttribute("data-quantity-variant-id") === cartUpsellVariantId
      );
    });
  ...
  async removeUpsellProduct(cartUpsellVariantId) {
    ...
    const upsellIndex =
      upsellItem.querySelector("input[data-index]").dataset.index;
